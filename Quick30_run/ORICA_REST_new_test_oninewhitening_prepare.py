...
def print_full(name, A):
    print(A.shape)    # (2, 3)
    print(A.dtype)    # float64 (对应 MATLAB double)
    print(A.nbytes)   # 总字节数
    print(A.itemsize) # 每个元素字节数 (8 bytes = 64 bit)


    #A = np.asarray(A, dtype=np.float64)   # 强制 float64
    # 十进制 17 位（round-trip）
    ...
    # IEEE754 十六进制位串（与 MATLAB num2hex 对齐）
...
def dynamic_whitening(blockdata, data_range, state, lambda_const,gamma,lambda_0):
    ...
    if lambda_values[0] < lambda_const:
        lambda_values = np.full(nPts_block, lambda_const)#这个nPts_block是未定义的，但是const=0
    ...
    # 增加详细的调试信息
    print(f"  🔍 白化更新详情:")
    print(f"    数据块形状: {blockdata.shape}")
    print("lambda_values",lambda_values)
    print(f"    遗忘因子: {lambda_avg:.6f}")
    print(f"    QWhite值: {QWhite:.6f}")
    print(f"    更新前白化矩阵[0,0]: {state['icasphere'][0,0]:.6f}")
    print(f"    更新项[0,0]: {update_term[0,0]:.6f}")
    print(f"    更新后白化矩阵[0,0]: {state['icasphere'][0,0]:.6f}")
    print(f"    白化矩阵变化量: {np.linalg.norm(update_term):.6f}")
...
def orica_rls_whitening(data, block_size_white=8, num_pass=1, 
    ...
    if verbose:
        ...
        print(f"数据形状: {data.shape}, 块大小: {block_size_white}, 块数量: {num_block}")
        print(f"参数: lambda_0={lambda_0}, gamma={gamma}, lambda_const={lambda_const}")
        import time
    ...
    for it in range(num_pass):
        ...
        for bi in range(num_block):
            ...
            if verbose and bi % 10 == 0:
                ...
                print(f"  数据块形状: {blockdata.shape}")
                #print(f"  遗忘因子范围: [{np.min(lambda_values):.6f}, {np.max(lambda_values):.6f}]")
                print(f"  白化矩阵[0,0]: {state['icasphere'][0,0]:.6f}")
                print(f"  计数器: {state['counter']}")
                print("  " + "-"*50)
...
if __name__ == "__main__":
    ...
    nChs, nPts = X.shape
    print(f"\n📊 数据信息:")
    print(f"   - 通道数 (nChs): {nChs}")
    print(f"   - 时间点数 (nPts): {nPts}")
...
'''
    ...
    print(f"白化后数据形状: {X_whitened.shape}")
    print(f"ORICA结果形状: {y_orica.shape}")
    print(f"白化后协方差矩阵形状: {cov_whitened.shape}")
    print(f"白化后协方差矩阵对角线元素:")
    for i in range(min(5, nChs)):
        print(f"  源{i+1}: {cov_whitened[i,i]:.6f}")
    ...
    try:
        ...
        print(f"💾 ORICA结果已保存到: {txt_orica_path}")
        ...
        print(f"💾 ICA权重矩阵已保存到: {txt_w_path}")
