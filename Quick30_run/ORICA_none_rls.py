...
def example_usage():
    ...
    orica = ORICA(
        ...
        adaptive_ff='cooling',
        nonlinearity='adaptive',
        sample_rate=500,
        block_size=8
    )
    
    # 初始化
    X_init = X[:100]  # 使用前100个样本初始化
    orica.initialize(X_init)
    
    # 在线处理
    X_stream = X[100:]  # 剩余数据用于在线处理
    Y = orica.fit_online(X_stream)
    
    print(f"输入数据形状: {X_stream.shape}")
    print(f"输出数据形状: {Y.shape}")
    print(f"收敛性指标: {orica.get_convergence_metrics()}")
    
    return orica, Y


if __name__ == "__main__":
    orica, Y = example_usage()