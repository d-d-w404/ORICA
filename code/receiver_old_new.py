...
from paths import DEFAULT_ASR_MAT, DEFAULT_ASR_NPZ_CALIB_LEGACY, OUTPUT_DATA_ROOT
...
class LSLStreamReceiver:
    def __init__(self, stream_type='EEG', time_range=5, stream_name='mybrain'):
        ...
        self.processed_save_file = None  # 处理后数据保存文件路径
        self.unprocessed_save_file = None  # 未处理数据保存文件路径
        self.processed_data_list = []  # 累积的处理后数据列表
        self.unprocessed_data_list = []  # 累积的未处理数据列表
    ...
    def find_and_open_stream(self):
        ...
        # ========================6) 如果启用保存，自动初始化保存文件===============================
        if self.save_processed_data and self.processed_save_file is None:
    ...
    def pull_and_update_buffer(self):
        ...
        if timestamps:
            ...
            if self.asr_filter is not None:
                ...
                print("asr_accum_buffer.shape",self.asr_accum_buffer.shape)
                ...
                if self.asr_accum_buffer.shape[1] >= target_len:
                    ...
                    print("asr_out.shape",asr_out.shape)
    ...
    def enable_processed_data_saving(self, save_file=None):
        ...
        if save_file is None:
            ...
            save_dir = OUTPUT_DATA_ROOT
            save_dir.mkdir(parents=True, exist_ok=True)
            # 生成两个文件路径
            processed_file = save_dir / f"eeg_orica1.npz"
            unprocessed_file = save_dir / f"eeg1.npz"
        else:
        ...
        self.processed_save_file = Path(processed_file)
        self.unprocessed_save_file = Path(unprocessed_file)
        print(f"✅ 已启用数据保存")
        print(f"   处理后数据文件: {self.processed_save_file}")
        print(f"   未处理数据文件: {self.unprocessed_save_file}")
        print(f"   数据为空后将等待 {self.data_empty_wait_time} 秒后完成保存")
    
    def disable_processed_data_saving(self):
        """禁用处理后数据保存功能"""
        if self.save_processed_data and len(self.processed_data_list) > 0:
            print("💾 保存已累积的数据...")
    ...
    def _append_processed_data_to_list(self, processed_chunk):
        """
        将处理后的chunk和未处理的chunk追加到数据列表
        
        Args:
            processed_chunk: 处理后的数据块 (所有通道, 样本数)
        """
        # 保存处理后的数据（只保存选中通道）
        processed_chunk_to_save = processed_chunk[self.chan_range, :]
        self.processed_data_list.append(processed_chunk_to_save.copy())
        
        # 保存未处理的数据（只保存选中通道）
        if self.last_unclean_chunk is not None:
            unclean_chunk_to_save = self.last_unclean_chunk[self.chan_range, :]
            self.unprocessed_data_list.append(unclean_chunk_to_save.copy())
        else:
            # 如果 last_unclean_chunk 为 None，使用处理后的数据（作为后备）
            print("⚠️ 警告: last_unclean_chunk 为 None，使用处理后的数据作为未处理数据")
            self.unprocessed_data_list.append(processed_chunk_to_save.copy())
    ...
    def _monitor_and_save_data(self):
        ...
        # 保存文件
        if len(self.processed_data_list) > 0:
            self._save_processed_data_to_file()
        else:
            print("⚠️ 没有数据需要保存")
    ...
    def _save_processed_data_to_file(self):
        ...
        try:
            # 合并所有chunk
            all_processed_data = np.concatenate(self.processed_data_list, axis=1)  # (channels, total_samples)
            
            # 合并未处理的数据
            all_unprocessed_data = None
            if len(self.unprocessed_data_list) > 0:
                # 检查未处理数据列表长度是否与处理后数据列表一致
                if len(self.unprocessed_data_list) == len(self.processed_data_list):
                    all_unprocessed_data = np.concatenate(self.unprocessed_data_list, axis=1)
                else:
                    print(f"⚠️ 警告: 未处理数据列表长度 ({len(self.unprocessed_data_list)}) 与处理后数据列表长度 ({len(self.processed_data_list)}) 不一致")
                    # 如果长度不一致，尝试合并已有的数据
                    if len(self.unprocessed_data_list) > 0:
                        all_unprocessed_data = np.concatenate(self.unprocessed_data_list, axis=1)
            
            # 保存处理后的数据到 eeg_orica.npz
            processed_save_dict = {
                'data': all_processed_data,  # 处理后的数据 (channels, samples)
                'channels': self.chan_labels,  # 通道名称
                'channel_indices': self.chan_range,  # 通道索引
                'sampling_rate': self.srate,  # 采样率
                'duration': all_processed_data.shape[1] / self.srate,  # 总时长（秒）
                'total_samples': all_processed_data.shape[1]  # 总样本数
            }
            
            np.savez(self.processed_save_file, **processed_save_dict)
            print(f"\n💾 处理后数据保存完成！")
            print(f"   文件: {self.processed_save_file}")
            print(f"   数据形状: {all_processed_data.shape}")
            print(f"   通道数: {all_processed_data.shape[0]}")
            print(f"   总样本数: {all_processed_data.shape[1]}")
            print(f"   总时长: {all_processed_data.shape[1]/self.srate:.2f}秒")
            
            # 保存未处理的数据到 eeg.npz
            if all_unprocessed_data is not None:
                unprocessed_save_dict = {
                    'data': all_unprocessed_data,  # 未处理的数据 (channels, samples)
                    'channels': self.chan_labels,  # 通道名称
                    'channel_indices': self.chan_range,  # 通道索引
                    'sampling_rate': self.srate,  # 采样率
                    'duration': all_unprocessed_data.shape[1] / self.srate,  # 总时长（秒）
                    'total_samples': all_unprocessed_data.shape[1]  # 总样本数
                }
                
                np.savez(self.unprocessed_save_file, **unprocessed_save_dict)
                print(f"\n💾 未处理数据保存完成！")
                print(f"   文件: {self.unprocessed_save_file}")
                print(f"   数据形状: {all_unprocessed_data.shape}")
                print(f"   通道数: {all_unprocessed_data.shape[0]}")
                print(f"   总样本数: {all_unprocessed_data.shape[1]}")
                print(f"   总时长: {all_unprocessed_data.shape[1]/self.srate:.2f}秒")
            else:
                print(f"\n⚠️ 未处理数据为空，未保存未处理数据文件")
    ...
    def initialize_asr_from_mat(self, mat_file_path=None):
        """
    ...
    def initialize_asr_from_npz(self, npz_file_path=None, cutoff=90):
        """
        ...
        if npz_file_path is None:
            npz_file_path = DEFAULT_ASR_NPZ_CALIB_LEGACY
        if self.asr_filter is not None:
            print("⏩ ASR 已校准，跳过重复初始化")
            return self.asr_filter
