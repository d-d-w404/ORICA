...
from paths import DEFAULT_ASR_MAT, DEFAULT_ASR_NPZ_CALIB, resolve_save_dir
...
class LSLStreamReceiver:
    ...
    #def __init__(self, stream_type='EEG', time_range=5, stream_name='CGX Quick-32r Q32r-0584'):
        ...
        # 在线 IIR 陷波（method 4：紧接带通 IIR 之后）；EEG_NOTCH_FREQ=0 关闭，默认 60（美标工频）
        _nf = os.environ.get("EEG_NOTCH_FREQ", "60").strip()
        try:
            self.notch_freq_hz = float(_nf) if _nf else 0.0
        except ValueError:
            print(f"[WARN] EEG_NOTCH_FREQ={_nf!r} 非法，陷波已关闭")
            self.notch_freq_hz = 0.0
        try:
            self.notch_q = float(os.environ.get("EEG_NOTCH_Q", "30"))
        except ValueError:
            self.notch_q = 30.0
        self.notch_filter_b = None
        self.notch_filter_a = None
        ...
        if self.notch_freq_hz > 0:
            print(
                f"[Notch] IIR method=4 时接在带通后: {self.notch_freq_hz:g} Hz, Q={self.notch_q:g} "
                f"(EEG_NOTCH_FREQ=0 关闭；欧标工频设 50)"
            )
    ...
    def find_and_open_stream(self):
        ...
        exclude = ['TRIGGER', 'ACC34','ACC33','ACC32', 'Packet Counter', 'ExG 2','ExG 1','ACC','vehicle position','A1','A2']#,'F7','F8'
    ...
    def process_orica(self, chunk, threshold=None):
        ...
        if self.orica is not None:
            # chunk 在 pull 里已裁剪为 len(chan_range)×samples；此处用连续行，勿再 chan_range 索引
            if self.orica.update_buffer(chunk):
    ...
    def pull_and_update_buffer(self):
        ...
        if timestamps:
            ...
            raw_chunk = chunk.copy()
            # chunk 已是选中通道子矩阵；勿再用全流索引 chan_range 二次索引 raw_chunk（会越界）
            # 每帧初始化四路阶段数据（后续分支会覆盖）
            ...
            if filter_method == '1':
                ...
                chunk = self.apply_online_iir_filter(chunk)
                chunk = self.apply_online_notch_filter(chunk)
                chunk_after_iir = chunk.copy()
    ...
    def _initialize_online_notch_filter(self, n_channels: int) -> bool:
        """二阶 IIR 陷波（iirnotch），与带通独立一套 b,a,zi。"""
        if self.notch_freq_hz <= 0:
            return False
        if self.srate is None:
            print("⚠️ 采样率未设置，无法初始化陷波")
            return False
        nyq = 0.5 * float(self.srate)
        if self.notch_freq_hz >= nyq - 0.5:
            print(
                f"⚠️ 陷波频率 {self.notch_freq_hz} Hz >= 奈奎斯特 {nyq:.1f} Hz，跳过陷波"
            )
            return False
        try:
            b, a = iirnotch(self.notch_freq_hz, self.notch_q, fs=float(self.srate))
            self.notch_filter_b = b
            self.notch_filter_a = a
            zi_single = lfilter_zi(b, a)
            self.notch_filter_zi = np.tile(zi_single, (n_channels, 1)).T
            print(
                f"✅ 在线陷波初始化: {self.notch_freq_hz:g} Hz, Q={self.notch_q:g}, "
                f"{n_channels} 通道"
            )
            return True
        except Exception as e:
    ...
    def apply_online_notch_filter(self, data: np.ndarray) -> np.ndarray:
        """带通 IIR 之后调用；chunk 间保持 lfilter 状态。关闭陷波时原样返回。"""
        if self.notch_freq_hz <= 0 or data is None or data.size == 0:
            return data
        try:
            n_channels, n_samples = data.shape
            if (
                self.notch_filter_b is None
                or self.notch_filter_a is None
                or self.notch_filter_zi is None
                or self.notch_filter_zi.shape[1] != n_channels
            ):
                if not self._initialize_online_notch_filter(n_channels):
                    return data
            assert self.notch_filter_b is not None and self.notch_filter_a is not None
            out_list = []
            zi_out = []
            for ch in range(n_channels):
                y_ch, zf_ch = lfilter(
                    ...
                    self.notch_filter_a,
                    data[ch, :],
                    zi=self.notch_filter_zi[:, ch],
                )
                out_list.append(y_ch)
                zi_out.append(zf_ch)
            self.notch_filter_zi = np.array(zi_out).T
    ...
    def enable_processed_data_saving(self, save_file=None):
        ...
        #   EEG_SAVE_DIR=output_data/asr20_2min_70/runA
        ...
        if save_file is None:
            ...
            if not save_dir_env:
                raise ValueError(
                    "缺少环境变量 EEG_SAVE_DIR（例如 output_data/asr20_2min_70/runA）。"
                )
            save_dir = resolve_save_dir(save_dir_env)
    ...
    def initialize_asr_from_mat(self, mat_file_path=None, cutoff=5):
        """
    ...
    # REPO_ROOT / "temp_txt/cleaned_data_quick30.npz"
    # QUICK30_RUN / "artifact_removal_verify/IIR_filter2/cali/laporoscopic_1309_EEGmerged_861_981.npz"
    def initialize_asr_from_npz_1(self, npz_file_path=None, cutoff=5):

    # def initialize_asr_from_npz(self, npz_file_path=DEFAULT_ASR_NPZ_CALIB_LEGACY, cutoff=90):
        """
        ...
        if npz_file_path is None:
            npz_file_path = DEFAULT_ASR_NPZ_CALIB
        if self.asr_filter is not None:
            print("⏩ ASR 已校准，跳过重复初始化")
            return self.asr_filter
    ...
    def initialize_asr_from_mat_1(self, mat_file_path=None, cutoff=5):
        """
    ...
    def _rows_for_selected_channels(self, arr):
        """
        可视化缓冲可能是：
        - (len(chan_range), T)：已与剔除后通道一致，勿再用全流索引 chan_range 取行；
        - (info.channel_count(), T)：全流，需 self.chan_range 取行。
        """
        if arr is None:
            return None
        a = np.asarray(arr)
        ...
        n_sel = len(self.chan_range) if self.chan_range else 0
        if n_sel > 0 and int(a.shape[0]) == n_sel:
            return np.ascontiguousarray(a)
        if self.chan_range is not None and len(self.chan_range) > 0:
            return np.ascontiguousarray(a[self.chan_range, :])
        return np.ascontiguousarray(a)
    ...
    def get_pair_data(self):
        ...
        pb = self.pair_buffer
        return (
            self._rows_for_selected_channels(pb[0]),
            self._rows_for_selected_channels(pb[1]),
            self._rows_for_selected_channels(pb[2]),
        )

    def get_pair_data_old(self, data_type='processed'):
        if self.chunk_pairs is None:
            return None
        cp = self.chunk_pairs.copy()[0]
        if data_type == 'raw':
            return self._rows_for_selected_channels(cp[1])
        return self._rows_for_selected_channels(cp[2])
    ...
    def get_buffer_data(self, data_type='processed'):
        ...
        """
        if data_type == 'raw':
            return self._rows_for_selected_channels(self.raw_buffer)
        return self._rows_for_selected_channels(self.buffer)
