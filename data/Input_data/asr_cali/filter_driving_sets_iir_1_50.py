"""
从 Input_data/npz/ 生成 ASR 校准数据，目录结构镜像 npz/：

  npz/Shawn_shared/foo.npz
    → asr_cali/Shawn_shared/full/foo.npz   （IIR 1–50 Hz 全长）
    → asr_cali/Shawn_shared/2min/foo.npz    （同上，仅前 2 分钟）
...
DEFAULT_INPUT_ROOT = SCRIPT_DIR.parent / "npz"
...
def process_one(
    ...
    *,
    ...
    if check_only:
        for out, kind in ((out_full, FULL_SUBDIR), (out_head, HEAD_SUBDIR)):
            exists = out.is_file()
...
def main() -> int:
    ap = argparse.ArgumentParser(description="npz/ → IIR 1–50 Hz → asr_cali/{subdir}/full|2min/")
    ...
    ap.add_argument("--force", action="store_true", help="覆盖已有 asr_cali 文件")
